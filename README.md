...
- 将硬编码的.java转为可编辑的.bbmodel：将需要转换的.java文件放在Model_Java文件夹下，运行ModelBencher.py后输出到Model_BB。如果是反编译后（未反混淆）的文件，则放入Model_Java_Obf文件夹，运行Model_Deobfer.py后输出到Model_Java_Deobf下，即可使用